...
fileName = sys.argv[1]
fitsName = ".".join(os.path.basename(os.path.normpath(fileName)).split('.')[:-1])
channeling = int(sys.argv[2])
features = sys.argv[3]

spectral_file_out = "spectrum_document.dat"
if 'expanded' in features:
    expansion = "_"+features.split('_')[-2]
else:
    expansion = ''

spectral_file_out_prev = "../spectrum_documents/"+fitsName+"_"+str(channeling)+expansion+".dat"

if os.path.isfile(spectral_file_out_prev):
    print("Copying document generated previously...")
    shutil.copyfile(spectral_file_out_prev, spectral_file_out)
    sys.exit(1)
...
mFile_out2 = open(spectral_file_out_prev,'w') #For saving the document

print("Generated File:")
print(" ".join(words)+"\n")

mFile_out.write(" ".join(words)+"\n")
mFile_out2.write(" ".join(words)+"\n")
