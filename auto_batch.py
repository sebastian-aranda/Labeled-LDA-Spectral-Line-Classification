...
if (len(sys.argv) != 2):
	print("e.g. python auto_batch.py test_data/data.csv")
	sys.exit(1)
	...
	with open(mData) as f:
		for line in f:
			...
			if (len(tokens) < 3):
				continue
			#print(tokens)
			os.system("python auto.py "+model+" "+str(channeling)+" "+tokens[2]+" casa_spectrums/"+tokens[1]+" "+labelmap)
        ...
        with open(mData) as f:
                for line in f:
                        ...
                        if (len(tokens) < 3):
                                continue
                        #print(tokens)
                        os.system("python auto.fix1.py "+model+" "+str(channeling)+" "+tokens[2]+" casa_spectrums/"+tokens[1]+" "+labelmap)
