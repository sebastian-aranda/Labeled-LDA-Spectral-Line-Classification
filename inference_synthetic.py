...
if (len(sys.argv) != 8):
	print("You must give 7 parameters: model_path, features_path, labels_path, channeling, fits_path, example_no, species_no")
	print("e.g. python inference_synthetic.py llda_models/model_hot_cores_full_500it/ llda_train_input/hot_cores_full_features.dat llda_train_input/hot_cores_full_labelmap.sub 5 ../Synthetic_Spectrums_125/Carbon_Monoxide/Carbon_Monoxide__COv\=0__1-0/trainingData.csv 0 68")
	sys.exit(1)
...
os.system("python synthetic2spectrum.py "+filename+" "+channeling+" "+"../llda_train_input/"+features+" "+example_no)
...
os.system("java -mx8192M -cp bin:lib/args4j-2.0.6.jar:lib/trove-3.0.3.jar jgibblda.LDA -inf -dir ../llda_models/"+model+"/ -model "+model+" -niters 500 -twords 10 -dfile "+temp_filename+".dat.gz")
